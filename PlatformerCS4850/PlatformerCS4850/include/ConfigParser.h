...
public:
	...
	GameObject* createGameObject(/** The parsed valueof the object from the level */int objectValue, 
		...
		switch (objectValue) {
			...
			case 2:
				enemy = new Enemy();
				enemy->init(x, y, ENEMY_WIDTH, ENEMY_HEIGHT, ENEMY_SPRITE);
				return enemy;
				break;
			case 3:
				battery = new Battery();
				battery->init(x, y, TILE_SIZE, TILE_SIZE, BATTERY_SPRITE);
				return battery;
				break;
			case 4:
				rocket = new Rocket();
				rocket->init(x, y, TILE_SIZE, TILE_SIZE, ROCKET_SPRITE);
				return rocket;
			default:
				return nullptr;
		}
